...
class FortiManager(object):
    ...
    def config_lock(self, module, msg="Unable to Lock the Configuration; Validate the ADOM is not Currently Locked."):
        ...
        if lock_status["result"][0]["status"]["code"] != 0:
            ...
            self.logout()
            module.fail_json(msg=msg, locked=False, saved=False, unlocked=False, fortimgr_response=lock_status)
    ...
    def config_response(self, module, json_response, lock):
        ...
        # attempt to unlock if config unsuccessful
            ...
            module.fail_json(msg="Unable to Apply Config", locked=True, saved=False, unlocked=True, fortimgr_response=json_response)
        # fail if not using lock mode and config unsuccessful
        elif status_code != 0:
            module.fail_json(msg="Unable to Apply Config", fortimgr_response=json_response)
    ...
    def config_save(self, module, msg="Unable to Save Config, Successfully Unlocked"):
        ...
        if save_status["result"][0]["status"]["code"] != 0:
            ...
            # try to logout before failing
            self.logout()
            module.fail_json(msg=msg, locked=True, saved=False, unlocked=True, fortimgr_response=save_status)
    ...
    def config_unlock(self, module, msg="Config Saved, but Unable to Unlock", saved=True):
        ...
        if unlock_status["result"][0]["status"]["code"] != 0:
            # try to logout before failing
            self.logout()
            module.fail_json(msg=msg, locked=True, saved=saved, unlocked=False, fortimgr_response=unlock_status)
    ...
    def get_package_installation_targets(self, adom, package):
        """
        This method is used to get all packages associated with an ADOM.
        ...
        :return: A list of package names associated with the ADOM. If the ADOM is not found or does not have any
                 packages, then an empty list is returned.
        """
        body = {"method": "get", "params": [{"url": "/pm/pkg/adom/{}/{}".format(adom, package)}], "id": 1,
    ...
    def add_package_installation_targets(self, adom, package, proposed):
        """
        This method is used to get all packages associated with an ADOM.
        ...
        :return: A list of package names associated with the ADOM. If the ADOM is not found or does not have any
                 packages, then an empty list is returned.
        """
        body = {"method": "add", "params": [{"url": "/pm/pkg/adom/{}/{}/scope member".format(adom, package), "data": [proposed]}], "id": 1,
                                              "session": self.session}
    ...
    def delete_package_installation_targets(self, adom, package, proposed):
        """
        This method is used to get all packages associated with an ADOM.
        ...
        :return: A list of package names associated with the ADOM. If the ADOM is not found or does not have any
                 packages, then an empty list is returned.
        """
        body = {"method": "delete", "params": [{"url": "/pm/pkg/adom/{}/{}/scope member".format(adom, package), "data": [proposed]}], "id": 1,
                                              "session": self.session}
    ...
    def get_device_package_status(self, adom, fortigate, vdom):
        """
        This method is used to get all packages associated with an ADOM.
        ...
        :return: A list of package names associated with the ADOM. If the ADOM is not found or does not have any
                 packages, then an empty list is returned.
        """
        body = {"method": "get", "params": [{"url": "/pm/config/adom/{}/_package/status/{}/{}".format(adom, fortigate, vdom)}], "id": 1,
                                              "session": self.session}
    ...
    def get_device_fields(self, device, fields=[], alladom=False):
        """
        ...
        :return: The json response from the request to retrieve the configured device. An empty list is returned if the
                 request does not return any data.
        """
        if not self.adom or alladom:
            dev_url = "/dvmdb/device"
        else:
            dev_url = "{}device".format(self.dvmdb_url)

        body = dict(method="get", params=[dict(url=dev_url, filter=["name", "==", device], fields=fields)],
                    verbose=1, session=self.session)
        response = self.make_request(body)
    ...
    def get_dynamic_interface(self, adom, interface):
        """
        This method is used to retrieve information about a dynamic interface from FortiManager.

        :param adom: Type str.
                       The name of the adom to retrieve information from.
        :param interface: Type str.
                       A name of the dynamic interface to retrieve for the adom.
        :return: The json response from the request to retrieve the configured device. An empty list is returned if the
                 request does not return any data.
        """
        config_url = "/pm/config/adom/{}/obj/dynamic/interface/{}".format(adom, interface)
        body = dict(method="get", params=[dict(url=config_url)], verbose=1, session=self.session)
        response = self.make_request(body).json()

        return response

    def add_dynamic_interface(self, adom, interfaec, proposed):
        """
        This method is used to create/add/set a dynamic interface on FortiManager.

        :param adom: Type str.
                       The name of the adom to retrieve information from.
        :param interface: Type str.
                       A name of the dynamic interface to retrieve for the adom.
        :param proposed: Type list (of dict).
                         The data portion of the API Request.
        :return: The json response from the request to retrieve the configured device. An empty list is returned if the
                 request does not return any data.
        """
        body = {"method": "set", "params": [{"url": "/pm/config/adom/{}/obj/dynamic/interface".format(adom), "data": [proposed]}], "id": 1,
                                              "session": self.session}

        response = self.make_request(body).json()
        return response

    def del_dynamic_interface(self, adom, interface):
        """
        This method is used to retrieve information about a dynamic interface from FortiManager.
        ...
        """
        config_url = "/pm/config/adom/{}/obj/dynamic/interface/{}".format(adom, interface)
        body = dict(method="delete", params=[dict(url=config_url)], session=self.session)
        response = self.make_request(body).json()

        return response

    # Where Used 
    def get_object_where_used(self, objecturl, objectid):
        search_where_used_start = "/cache/search/where/used/start"
        body = {"method": "exec", "params": [{"url": search_where_used_start, "data": {"obj": objecturl, "mkey":objectid}}], "id": 1,
                                              "session": self.session}
        response = self.make_request(body).json()
        if response["result"][0]["status"]["code"] == 0:
            search_where_used_token = response["result"][0]["data"]["token"]
        else:
            return "start where used search failed", response
        

        search_where_used_summary = "/cache/search/where/used/get/summary"
        body = {"method": "exec", "params": [{"url": search_where_used_summary, "token": search_where_used_token}], "id": 1,
                                              "session": self.session}
        search_where_used_done = False
        while not search_where_used_done:
            response = self.make_request(body).json()
            if response["result"][0]["status"]["code"] == 0:
                if response["result"][0]["data"]["percent"] == 100:
                    search_where_used_done = True
                else:
                    time.sleep(1)
            else:   # can't get search where used summary, something wrong
                return "get where used search summary failed", response

        search_where_used_detail = "/cache/search/where/used/get/detail"
        body = {"method": "exec", "params": [{"url": search_where_used_detail, "token": search_where_used_token}], "id": 1,
                                              "session": self.session}
        response = self.make_request(body).json()

        search_where_used_end = "/cache/search/where/used/end"
        body = {"method": "exec", "params": [{"url": search_where_used_end, "token": search_where_used_token}], "id": 1,
                                              "session": self.session}
        self.make_request(body).json()

        if response["result"][0]["status"]["code"] == 0:
            return 0, response
        else:
            return "get where used search detail failed", response
    ...
    def get_task(self, task, wait):
        ...
        while percent_complete != 100 and state_done != True:
            response = self.make_request(body).json()
            if response["result"][0]["status"]["code"] == 0:
                if response["result"][0]["data"].get("percent"):
                    percent_complete = response["result"][0]["data"]["percent"]
                else: # I noticed that in some case, the response["result"][0]["data"] is missing this "percent" field
                    percent_complete = 0
                if response["result"][0]["data"].get("state"):
                    state_done == response["result"][0]["data"]["state"]
                else:
                    state_done = False

            # limit execution time to specified time in minutes
            if time.time() - countdown > wait * 60:
                break
            else:
                time.sleep(15)
    ...
    def add_device(self, proposed):
        """
        This method is used to add a managed device to FortiManager.

        :param proposed: Type list.
                         The data portion of the API Request.
        :return: The json result data from the task associated with request to make add the device.
        """
        body = {"method": "exec", "params": [{"url": "/dvm/cmd/add/device", "data": proposed, "id": 1,
                                              "session": self.session}]}
        ...
        # collect task id
        if response["result"][0]["status"]["code"] == 0:
            task = response["result"][0]["data"]["taskid"]
        else:
            return response

        # check for task completion
        task_status = self.get_task(task, 15)

        return task_status

    def update_ha(self, proposed):
        """
        This method is used to update device in a managed HA cluster on FortiManager.

        :param proposed: Type list.
                         The data portion of the API Request.
        :return: The json result data from the task associated with request to make add the device.
        """
        body = {"method": "exec", "params": [{"url": "/dvm/cmd/update/ha", "data": proposed, "id": 1,
                                              "session": self.session}]}
    ...
    def del_device(self, proposed):
        ...
        :return: The json result data from the task associated with request to make add the device.
        """
        body = {"method": "exec", "params": [{"url": "/dvm/cmd/del/device", "data": proposed, "id": 1,
                                              "session": self.session}]}
    ...
    def del_devices(self, proposed):
        ...
        :return: The json result data from the task associated with request to make add the device.
        """
        body = {"method": "exec", "params": [{"url": "/dvm/cmd/del/dev-list", "data": proposed, "id": 1,
                                              "session": self.session}]}
    ...
    def apply_method_on_script(self, method, proposed):
        """
        This method is apply get/set/add/update/delete/exec method on a script to FortiManager.

        :param proposed: Type list.
                         The data portion of the API Request.
        :return: The json result data from the task associated with request to make add the script.
        """
        if not self.adom:
            devdb_script_url = "/dvmdb/script/"
        else:
            devdb_script_url = "{}script/".format(self.dvmdb_url)

        if method == "exec":    # execute script will create a task
            devdb_script_url = devdb_script_url + "execute"
            body = {"method": method, "params": [{"url": devdb_script_url, "data": proposed}], "id": 1, "session": self.session}
        else:
            body = {"method": method, "params": [{"url": devdb_script_url, "data": proposed, "id": 1, "session": self.session}]}
    ...
    def install_package(self, proposed):
        ...
        This also will install a device configuration to the end devices.
    ...
    def install_device(self, proposed):
        """
        This method is used to install a device configuration to the end devices.

        :param proposed: Type list.
                         The data portion of the API Request.
        :return: The json result data from the task associated with request to make install the package.
        """
        body = {"method": "exec", "params": [{"url": "/securityconsole/install/device", "data": proposed, "id": 1,
                                              "session": self.session}]}
        ...
        if module.params["direction"]:
            ...
            if not module.check_mode:
                    ...
                    if save_status["result"][0]["status"]["code"] == 0:
                        ...
                        if unlock_status["result"][0]["status"]["code"] != 0:
                            results.update(dict(locked=True, saved=True, unlock=False, moved=move,
                                                ...
                                                fortimgr_response=unlock_status))
                            module.fail_json(**results)
                    else:
                ...
                # do not attempt to save if unsuccessful move, but try to unlock before failing
                    ...
                    if unlock_status["result"][0]["status"]["code"] == 0:
                        results.update(dict(locked=True, saved=False, unlocked=True,
                                            ...
                                            fortimgr_response=response.json(), request_body=response.request.body))
                        module.fail_json(**results)
                    else:
                        results.update(dict(locked=True, saved=False, unlocked=False,
                ...
                # fail module when move unsuccessful and not in lock mode
                    ...
                    results.update(dict(msg="Policy Move Failed", fortimgr_response=response.json(),
